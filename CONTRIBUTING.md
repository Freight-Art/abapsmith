...
live integration suites — eighteen as of this writing — are excluded by name
...
`npm run test:live` (`VITEST_LIVE=1 vitest run`) runs the eighteen live suites in
...
debugger attach against that system. All of them collect under `ABAP_URL`
alone, but the ones that write (`integration-undo`, `integration-fpm-lock`,
`integration-class-includes`, `integration-lock-handle`,
`integration-checkfail-method-repair`, the `integration-fluid-*` and
`integration-ddic-structured` suites) additionally need
