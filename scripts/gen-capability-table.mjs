...
export async function buildCapabilityTable(registry) {
  ...
  const rows = Object.entries(registry).map(([type, cap]) => {
    ...
    return {
      ...
      // EXCEPT the three now excluded there because they have a working
      // catalog-based read (src/adt/catalog-query.ts + catalog-read.ts) —
      // SHLP/DH, VIEW/DV, TRAN/T. This script is plain Node with no
      // TypeScript loader, so it can't import src/adt/ddic.ts's
      // DDIC_CATALOG_BASED to derive this the way capabilities.ts does; the
      // three-code duplicate below is deliberate and cheap, and the census
      // test asserts buckets.nonReadable equals NON_READABLE_TYPES exactly,
      // so the two lists can't silently drift apart.
      nonReadable:
