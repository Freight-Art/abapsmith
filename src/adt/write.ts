...
/**
 * The lock names a DIFFERENT request from the one this mutation is about to
 * send — CTS records the change against `lock.corrNr` (the request that
 * already holds the object's enqueue), never against `corr.corrNr`, so this
 * is the one condition under which sending `corr` would be dishonest about
 * where the change lands. A `local` `corr`, or a lock that is itself local
 * or names nothing, is never divergent. Shared by `writeObject`'s PUT-path
 * refusal (`transportDivergence`) and `deleteObject`'s (`corrNrNotHonoured`),
 * so the one rule is written once rather than kept in sync by hand.
 */
export function divergentLockCorrNr(corr: WriteCorr, lock: TransportInfo): string | undefined {
  if (
    corr.kind === "transport" &&
    lock.required &&
    lock.corrNr !== undefined &&
    lock.corrNr !== "" &&
    lock.corrNr.toUpperCase() !== corr.corrNr.toUpperCase()
  ) {
    return lock.corrNr;
  }
  return undefined;
...
export function corrNrNotHonoured(
  ...
  return new AbapError(
    ...
    `CTS records a change on the request that already holds the object; a second request ` +
      ...
      `judged by the safety gate like any other), or remove the object's entry from ` +
      `${lockCorrNr} first with abap_transport operation removeObject (ABAP_MODE=admin) and ` +
      `retry with ${named}. Splitting create and delete across two requests does not work — ` +
      `see doc/LIMITATIONS/editing.md.`,
  );
}
...
export async function deleteObject(
  ...
  transport: TransportInfo;
  /** The number this DELETE actually put on the wire as `corrNr`; absent when it sent none (a local delete, or no transport manager wired). */
  corrNrSent?: string;
  /** Whether `corrNrSent` is the request the change was recorded in: `false` when the lock named a DIFFERENT request, which is the one CTS records the deletion on (`transport.corrNr`). Absent when the lock named no request at all, so the two could not be compared — absent is "not known", never "honoured". */
  corrNrHonoured?: boolean;
  /**
  ...
  if (isPackageType(t.type)) {
    ...
    //
    // This route takes no lock of its own (the generated ABAP does its own
    // enqueue server-side) — there is no `TransportInfo` from a lock to
    // compare `corrNr` against, so `corrNrSent`/`corrNrHonoured` can never be
    // known here and stay absent.
    return {
      target: t,
      deleted: true,
  ...
  // Set right before the DELETE is issued, same hoist-then-assign — what
  // actually went on the wire, and whether the lock agreed it would be
  // recorded there. See the return type's doc comments for what each means.
  let corrNrSent: string | undefined;
  ...
  await conn.withStatefulSession(async (session) => {
    ...
    // The lock is already held by a DIFFERENT request than `corr` — CTS will
    // record this deletion on `divergent`, not on `corr.corrNr`, no matter
    // what this call sends. See `divergentLockCorrNr`.
    const divergent = divergentLockCorrNr(corr, transport);
    if (divergent !== undefined && corr.kind === "transport") {
      if (corr.source === "named") {
        // A human (or a pinned/allowlisted ABAP_ALLOW_TRANSPORTS) named this
        // number; it can never be honoured for a delete, so refuse before
      ...
      // But `divergent` is the lock's request, not `corr.corrNr`, so
      // `preflightCorr`'s `gate.assert` above judged the WRONG number — the
      // one CTS will actually use has never been judged. Judge it now,
      // mirroring `preflightCorr`'s own `gate.assert` call. Under the default
      // ABAP_ALLOW_TRANSPORTS=["auto"] this permits any number, so in
      // practice this only bites a pinned/allowlisted configuration.
      if (opts.transport !== undefined) {
        try {
          opts.gate.assert(
            ...
            {
              corr: { kind: "transport", corrNr: divergent, source: "auto" },
              intent:
                isEnhancementType(t.type) && opts.affects
                  ? enhancementIntentFor(t, opts.affects)
                  : undefined,
          ...
          // A refusal here must release the enqueue first, same as every
          // other throw inside this session callback.
          await session.unlock(t.uri);
          throw e;
        }
      }
    ...
    // What actually goes on the wire, and whether the lock agreed it would
    // land in that request — only answerable when the lock named one at all
    // (`transport.corrNr` non-empty); otherwise there was nothing to compare
    // against, so `corrNrHonoured` stays `undefined` (unknown, not honoured).
    if (corr.kind === "transport") {
      corrNrSent = corr.corrNr;
      if (transport.required && transport.corrNr !== undefined && transport.corrNr !== "") {
        corrNrHonoured = divergent === undefined;
      }
    }
  ...
  return {
    ...
    previousSource,
    transport: transportInfo,
    ...(corrNrSent !== undefined ? { corrNrSent } : {}),
