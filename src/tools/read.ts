...
export const readInputSchema = {
  ...
  view: z
    .enum(["history", "diff", "definition", "lineage", "footprint"])
    .optional()
    .describe(
      'history: versions. diff: hunks. definition: element at line/column. lineage: CDS view sources ' +
        "down to base tables. footprint: database writes and commits. Omit for normal read.",
    ),
...
function assertViewCompatible(input: ReadInput, obj: ResolvedObject): void {
  ...
  // rather than inheriting the history/diff or definition wording.
      ...
      isDefinition
        ...
        : isLineage
          ...
          : isFootprint
            ...
            : 'the active/inactive pair is a different axis from the version FEED; "inactive" is not ' +
              "a feed entry and has no history row.",
      isDefinition
  ...
  // view (history/diff, and now lineage/footprint) is refused here, each
  // with its own honest reason rather than the history/diff wording
  // stretched to cover a tree or a whole-object scan too.
  if (!isDefinition) {
    for (const [param, value] of [
      ["line", input.line],
      ["column", input.column],
