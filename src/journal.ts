...
// `systemKey()` moved to src/system-key.ts (pool.ts needs it without the
// journal's `node:fs` reach); re-exported here so existing importers are
// unaffected.
export { systemKey } from "./system-key.js";
