 ...
 * element-info.ts) from the same flat row shape `refs()` below already
 * produced for the old, now-removed `conn.adt.usageReferences()` stub.
 * Round-tripped through `parseUsageReferences` this reproduces those exact
 * rows, so it is the wire-level fake `fetchUsageReferences` needs now that
 * `whereUsed` reads through `conn.post` instead of the vendor helper.
 */
...
function searchConn(handlers: {
  ...
  return {
    ...
    // which is one conn.post, not conn.adt.usageReferences() — see this
    // file's header comment and element-info.ts's fetchUsageReferences doc
    // comment for why the vendor helper is no longer trustworthy on A4H.
    post: async () => {
