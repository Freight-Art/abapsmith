 ...
 * for a type whose registry entry is `"unverified"` (DDLA/ADF and ENQU/DL,
 * both source/properties-shape and writable, whose CREATE could not be
 * gotten to succeed live, so delete was never reachable to test). The gate
 * also refuses a registry entry of `false`, but none exists today — after
 * the BDEF/BDO delete flip, no type in the registry is `delete: false`. A
 * type verified live (CLAS/OC) is checked as a same-shape control so this
 * isn't accidentally refusing everything.
 */
