/**
 * Offline fluid-protocol fake for the builtin `scan` tool (source-text scan,
 * entry class `ZCL_ZMCP_FLUID_SCAN`), modeled on
 * test/helpers/fluid-fpm-fake.ts's `fpmFluidRoute` (itself modeled on
 * test/fluid-classic.test.ts's `dynamicFluidRoute`) — an auto-vivifying class
 * store, unfiltered by class name (only two classes are ever deployed here:
 * the shared runtime and ZCL_ZMCP_FLUID_SCAN), serving the doc/source/
 * activation/classrun mechanics `dispatch()`/`ensureFluidTool` need.
 * Duplicated rather than imported, per this repo's per-file offline harness
 * convention (see fluid-classic.test.ts's own header).
 *
 * Adapted to a `RecordingClient` whose `respond` callback returns a
 * `HttpClientResponse` directly rather than composing `Route`s with `??` — so
 * `scanFluidRoute` still returns `undefined` for anything it doesn't
 * recognize, and callers fall back to their own base route (session/
 * discovery/ato) the same way `fluid-classic.test.ts` does.
 */
...
export type ScanRoute = (o: HttpClientOptions) => HttpClientResponse | undefined;
...
export function scanFluidRoute(opts: { transcript: () => string }): { route: ScanRoute; store: Map<string, ObjState> } {
  ...
  const route: ScanRoute = (o: HttpClientOptions) => {
...
/**
 * Builds a fluid console transcript for the `scan` tool: one `OUT` frame per
 * element of `outs` — hit rows followed by exactly one trailing summary row,
 * per `mapScanRows`'s (src/adt/source-scan.ts) own protocol.
 */
export function scanTranscript(opts: { ver: string; outs: readonly unknown[] }): string {
  const lines: string[] = [];
  lines.push(frameLine("BEGIN", { id: "scan", ver: opts.ver, action: "source", contract: "1.0" }));
...
export function scanTruncatedTranscript(opts: { ver: string; outs: readonly unknown[] }): string {
  const lines: string[] = [];
  lines.push(frameLine("BEGIN", { id: "scan", ver: opts.ver, action: "source", contract: "1.0" }));
  for (const v of opts.outs) lines.push(frameLine("OUT", v));
...
/** A transcript reporting one ERR frame and END rc=1 — a scan-side failure. */
export function scanErrTranscript(opts: {
  ver: string;
