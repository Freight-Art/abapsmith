...
the seven live integration suites by name from every run that is not
...
screen, a write into a class's CCAU include, a lock-handle validity
check, and a CHECK_FAILED-then-`method=`-repair sequence against a class's
inactive version. All seven require `ABAP_URL` to collect at all; five of
them (undo, FPM-lock, class-includes, lock-handle, checkfail-method-repair)
