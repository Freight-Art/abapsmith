...
| `abap_write text_pool` entries | refused unconditionally, unforceable | `irreversible: true`, refused by `undoBlocker()`'s generic catch-all — same branch as the BOPF and enhancement entries above. The previous symbols/selection texts are not captured as a before-image; rewrite them with another `text_pool` call to revert. |
