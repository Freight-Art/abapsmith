...
| BOPF entries (`abap_bopf_edit`, `abap_bopf_delete`) | mixed | An `abap_bopf_edit update` entry (`beforeKind: "bopf-model"`) records the previous model XML; undo PUTs it back and re-activates. BOPF PUT re-mints node GUIDs, so the restored model matches the before-image at model level, not byte-for-byte. `create_bo` and `abap_bopf_delete` stay not undoable — both use non-atomic APIs, and their entries say so at write time. |
