...
| `actor` | who made the change — `ABAP_ACTOR` if set; otherwise, under `ABAP_MCP_TRANSPORT=http`, the name of the `ABAP_MCP_HTTP_TOKEN` entry that authenticated the request, when it was a *named* token, else that session's MCP client `clientInfo.name`; under stdio (the default), the MCP client's `clientInfo.name` for the one process-wide session. Never a placeholder: an entry with no known actor simply omits the field, and `abap_journal mode=list` drops the `actor` column entirely on a page where nothing has one. See `ABAP_ACTOR` in [doc/CONFIGURATION § Journal, diagnostics, and tooling](../CONFIGURATION/journal-diagnostics-and-tooling.md), and [doc/SAFETY/remote-transport.md](../SAFETY/remote-transport.md) for what a token NAME does and does not authenticate. |
| `sessionId` / `sessionIdSource` | which conversation made the change — a different question from `actor`, split into its own field. Under `ABAP_MCP_TRANSPORT=stdio` (the default) this is still a UUID generated once per server process, `sessionIdSource: "process"` — one process is one client connection under stdio, so that id genuinely identifies "this conversation". Under `ABAP_MCP_TRANSPORT=http` it is the transport's own `Mcp-Session-Id`, minted by the SDK on that session's `initialize` (`sessionIdSource: "transport"`), so entries written by concurrent MCP sessions inside one process stay distinguishable from each other — see [doc/CONCURRENCY/mcp-sessions-and-the-pool.md](../CONCURRENCY/mcp-sessions-and-the-pool.md). Not a `mode=list` column (a rarely-read opaque id is better as a selector than as width) — filter with `abap_journal mode=list session=<id>`, or `session=current`, which resolves to the id of the session making the `abap_journal` call itself (the one running process under stdio; the calling MCP session under http). |
