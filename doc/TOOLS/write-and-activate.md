...
| `corr_nr` | string | no | — | Transport request to write into. Omit for `$TMP`-local objects. Required for a `VIEW/DV` create into a transportable package; refused for one into a `$` package. Also refused for a `VIEW/DV`/`TRAN/T` delete — neither delete bridge takes a transport parameter, and none is needed: the delete registers nothing in CTS, so it is judged as a local mutation regardless of `ABAP_ALLOW_TRANSPORTS`. |
