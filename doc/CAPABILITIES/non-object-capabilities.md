...
| Object activation | n/a | n/a | n/a | n/a | yes | live | Check-only and activate modes, single and batched. There is no deactivate in ADT; undoing an activation entry instead undoes the latest earlier succeeded write entry for the same object, restoring and re-activating it. |
...
| Transport requests | yes | yes | partial | yes | n/a | live | Create — a workbench request, or, with `kind: "copies"` and a required `target`, a transport of copies — add a user, and set an owner. A transport of copies is a snapshot to `target` with no tasks of its own, so `addUser` on one fails (`TRANSPORT_ERROR "I::000"`, `ADT_TM_COMMON_EXCEPTION`); the original request and its objects are untouched by the copy. Delete is admin-gated and requires echoing the request identifier, and also works on a transport of copies. Objects cannot be added or removed directly, and a locked entry cannot be unlocked. Transport-of-copies creation was proven by running the bridge ABAP directly from a throwaway `$TMP` probe class, and `show`, `log`, the `addUser` refusal, `delete`, and a follow-up `list` no longer showing it were proven through the released tool path (A4H, 2026-09-15); `setOwner` has its own separate live wire capture. |
| Transport release | n/a | yes | n/a | n/a | yes | live | Dry run by default, armed only by echoing the request identifier, and gated separately from ordinary write access. Reports four distinct outcomes and never overstates one. Verifying whether a release's export actually reached the target system now has read support alongside it — see the Transport log / Transport import queue rows below — but release itself still only reports whether CTS accepted the release, never `tp`'s import outcome downstream; nothing triggers that import. |
| Transport log | n/a | yes | n/a | n/a | n/a | mixed | `abap_transport operation="log"`. The overview (`TRINT_GET_LOG_OVERVIEW`, one row per target system) is `live`: it answers `sy-subrc 0` even for a request that does not exist, so abapsmith checks E070 first and returns `NOT_FOUND` for an unknown request rather than trusting the FM's always-succeeding answer. Log line content (`TRINT_GET_LOG_FILE`, the `tp` log itself) is `tests`-only — A4H has never run `tp`, so every live call returned zero lines. |
| Transport import queue | n/a | yes | n/a | n/a | n/a | mixed | `abap_transport operation="queue"` reads a target system's TMS import buffer (`TMS_MGR_READ_TRANSPORT_QUEUE`/`TMSBUFFER`), not an import history — a request that already imported has left the buffer. An empty-buffer read (with and without `domain`, the omitted domain rendered as the `-` placeholder) and an unknown-system refusal (`READ_CONFIG_FAILED`, `sy-msgid XT` `sy-msgno 126`, mapped to `NOT_FOUND`) are `live`; a non-empty queue and the rendering of its entries are `tests`-only, since nothing has ever actually been exported from the reference system. No operation triggers an import from the queue — see [doc/LIMITATIONS/not-implemented-and-unproven.md](../LIMITATIONS/not-implemented-and-unproven.md). |
...
| Undo | n/a | n/a | yes | yes | n/a | mixed | Reverts one journal entry. Undoes an `activate` entry via the preceding write; undoes `create_spot`/`create_impl`/`create_hook` (when confirmed-absent) and `set_impl_active`; undoes a BOPF `update`; undoes a text pool write. Refuses transport release, `add_badi_def`/`add_filter_def`/`set_filter_values`/`write_description`/enhancement `delete`, BOPF `create_bo`/`abap_bopf_delete`, and every stored `undoable: false` entry, with no override. Class-delete recreate (with its four sub-includes) and sub-include-targeted restore are `live` (SAP A4H, 2026-09-12); ordinary `main`-source restore and create-delete are `tests`-only. See the "Journal and undo" note below. |
...
| Data snapshot / diff | yes | yes | no | no | n/a | tests | `abap_data_preview mode="snapshot"`/`mode="diff"`, and `snapshot_ids` on `abap_run`/`abap_test`/`abap_bopf_test`/`abap_ui mode="press"`. Same gate and policy as an ordinary preview, re-checked again at diff time because the deny-list can grow between the two calls. Rows are matched on the DDIC primary key when known; a `columns` projection makes key completeness unprovable, so matching falls back to the selected columns and an edited row then shows as delete+insert, not a change — a structural limit of matching on full-row identity, not a bug. `ttl_hours` is clamped down, never up, to an operator ceiling (`ABAP_DATA_SNAPSHOT_TTL_HOURS`); an expired snapshot is pruned and `diff` refuses it as terminal. No Delete: a snapshot is removed only by TTL expiry, never by an explicit delete call. Stored under `ABAP_STATE_DIR` in `snapshots/<system>/`, mode `0600`, entirely separate from the write journal — it never surfaces via `abap_journal` or a journal export. See [diagnostics.md](../TOOLS/diagnostics.md) and [execute-and-test.md](../TOOLS/execute-and-test.md). |
| Change document reads | n/a | yes | n/a | n/a | n/a | tests | `abap_fluid {"tool":"core","action":"change_docs"}`. Reads `CDHDR`/`CDPOS` directly for one object class. Gated twice: `guardCoreAction` judges `CDHDR`/`CDPOS` themselves, by the same `ABAP_ALLOW_DATA_PREVIEW`/deny-list policy as `abap_data_preview`, before any ABAP runs; a second pass (`applyPositionPolicy`) then judges every table a returned `CDPOS` row actually names, after the read, since those tables are not known beforehand — a position naming a denied table is dropped and counted, never shown, while the change document it belongs to still appears. Rows are clamped to the same row ceiling as `abap_data_preview`. This closes the position-level half of a previously-documented gap; a plain `abap_data_preview` read of `CDPOS` itself remains unfiltered — see [data-access-and-credentials.md](../SAFETY/data-access-and-credentials.md). |
| Enqueue locks (generic) | n/a | yes | n/a | n/a | n/a | tests | `abap_fluid {"tool":"core","action":"locks"}`, via `ENQUEUE_READ`. Requires at least one of `object`/`table`/`user` — an empty call is refused as `BAD_INPUT` before any network call, so there is no way to dump the whole enqueue table through this action. Filters are matched client-side with a `CP` wildcard, since `ENQUEUE_READ`'s own filter parameters are exact-match only. Read-only: there is no release/`DEQUEUE` path anywhere in this action. Diagnostic fields (`GTCODE`/`GTHOST`/`GTDATE`/`GTTIME`) are probed at runtime and reported absent when a system's `SEQG3` does not carry them; **unverified**: whether `ENQUEUE_READ` actually fills these fields when present — only their presence as `SEQG3` components is established, not their runtime content. Also backs best-effort lock-holder enrichment on a `LOCKED` refusal from `abap_write`/`abap_activate` when ADT itself named no blocking user, attached as `details.lock_holders`; the lock-argument pattern that lookup searches on is an **inference** from `SEQG3` field semantics (`src/adt/locked-holders.ts`), not checked against an ADT-held lock. A failed enrichment lookup always returns the original `LOCKED` error unchanged. Distinct from `abap_fpm_read mode="locks"`, which stays FPM-config-specific and was deliberately not widened — see [ui-and-fpm.md](../TOOLS/ui-and-fpm.md). |
...
| Ad hoc ABAP snippet (`core.eval`) | n/a | n/a | n/a | n/a | yes | tests | Runs caller-supplied ABAP statements as the body of one generated method and serialises named local variables back as JSON, via the fluid API's `core` tool. Off by default (`ABAP_ALLOW_FLUID_EVAL`, not implied by any `ABAP_MODE` including `admin`), needs a per-call `confirm: "core.eval"` echo, and every call is journalled with the full, untruncated snippet, irreversibly. The static review and capability scan run per call, but this is a lint, not a sandbox: they reject a handful of named statements and do not confine what runs under the connected user's own authorisations. See [doc/FLUID-API/README.md](../FLUID-API/README.md) and [doc/FLUID-API/safety.md](../FLUID-API/safety.md). |
| UI automation | n/a | yes | n/a | n/a | yes | mixed | Classic dynpro only, driven by generated batch input. `mode=screen` can also render a monospace layout picture of the screen (`layout: true`), built client-side from the same read — design-time from `D021S`, not a runtime screenshot, and not compared against a live SAP GUI capture. Pressing commits immediately with no dry run and no rollback. |
...
- **Journal and undo.** The journal records writes, transport operations,
  ...
  recreate a delete, undo an `activate` entry via its preceding write, undo
  an undoable enhancement or BOPF-update entry, and restore a text pool.
  `force` overrides drift, and nothing else — it cannot manufacture the
  positive absence evidence a create-undo needs, and it does not override
  the transport-release, cross-system, class-include, or any stored
  `undoable: false` refusal, nor the live dependency/active-implementation
  checks an enhancement or activate undo still runs. A class delete now
