...
| Transport import queue | n/a | yes | n/a | n/a | n/a | mixed | `abap_transport operation="queue"` reads a target system's TMS import buffer (`TMS_MGR_READ_TRANSPORT_QUEUE`/`TMSBUFFER`), not an import history — a request that already imported has left the buffer. An empty-buffer read and an unknown-system refusal (`READ_CONFIG_FAILED` mapped to `NOT_FOUND`) are `live`; a non-empty queue and the rendering of its entries are `tests`-only, since nothing has ever actually been exported from the reference system. No operation triggers an import from the queue — see [doc/LIMITATIONS/not-implemented-and-unproven.md](../LIMITATIONS/not-implemented-and-unproven.md). |
