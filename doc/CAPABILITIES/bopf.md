...
| Alternative key | partial | yes | partial | yes | n/a | mixed | `add_alternative_key` is live-confirmed to write: `checkAfterModify`/`checkBeforeSave`/`noCheck` map onto one server-side field, so at most one may be `true`; `checkBeforeSave: true` is refused outright; `unique`/`uniqueIfNotInitial` require exactly one of `noCheck`/`checkAfterModify`; `notUnique` refuses `checkAfterModify`. Refused client-side as `BAD_INPUT`, not overridable by `allow_dangling_ref`, since the server-side arms missing those cases are what short-dumped the ADT session. No key added this way has been observed to activate — a `TABL/DS` `dataTypeRef` draws a severity-E message on `bo:dataTypeRef`, a `DTEL/DE` one draws no message, but `activate` then returns `activated: false` with zero activation messages, and removing the key restores `activated: true` — so `i_know_this_may_not_activate: true` is still demanded. `set_alternative_key_fields` patches xmlName, uniqueness, checkAfterModify, checkBeforeSave, noCheck, objectModelGenerated, dataTypeRef and dataTableTypeRef in place, checks the same check-mode rule against the effective post-patch state, and demands the same acknowledgement; `keyElements` and renaming (`name`) are refused. Add/remove attempts are live; `set_alternative_key_fields` is covered by tests only. |
