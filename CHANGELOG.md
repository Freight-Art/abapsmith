...
- BOPF `abap_bopf_edit` model updates record the previous model XML, and `abap_journal mode=undo` PUTs it back and re-activates (#200).
...
- **`abap_activate mode=check` checks a draft with no `object`** (#213). `{ mode: "check", type: "PROG/P" | "CLAS/OC" | "INTF/OI", source: "<draft>" }`, read-only, no lock, works under `ABAP_MODE=read` — the same as the existing object-form check. A `PROG/P` draft is checked against no server object at all (its report name comes from its own `REPORT`/`PROGRAM` statement; a draft missing that statement is reported by the server as a finding on line 1, not refused locally). A `CLAS/OC` or `INTF/OI` draft is checked as a draft of the EXISTING server object named in its own `CLASS ... DEFINITION`/`INTERFACE` statement — if that object doesn't exist yet the call returns `NOT_FOUND` rather than running the check, because ADT would otherwise report a draft against a nonexistent class as clean without actually processing it; write the object first with `activate: false`, or check plain report code as `PROG/P` instead. Any other `type` is `UNSUPPORTED` naming the three supported types, and a missing `type` is `BAD_INPUT` — both zero-network. Findings keep the object-check shape (`line`, `column`, `severity`, `message` under `# SYNTAX CHECK`; header `object`, `mode: check`, `inline: true`, `result`, `errors`, `warnings`). Verified live on A4H 2026-09-23: `{ mode: "check", type: "PROG/P", source: "REPORT zas_213.\nWRITE 'x'." }` answered clean (`object: PROG/P ZAS_213`, `inline: true`); the same draft with `WRITE x.` answered one error `line 2 col 6 Field "X" is unknown.`; a draft with no `REPORT` statement answered one error on line 1; a `CLAS/OC` draft of the existing inactive class `ZCL_AS_T217` answered one error (`line 7 col 18 Type "ZAS_T217" is unknown`) and wrote nothing; a `CLAS/OC` draft of a nonexistent class answered `NOT_FOUND`; `type: "TABL/DT"` answered `UNSUPPORTED`; the `PROG/P` check also ran under `ABAP_MODE=read`.
- **Inactive objects: package-scoped listing, package activation, and inactive-dependency detail on a failed activation** (#217). `abap_search { inactive: true, packages, include_subpackages?, query?, type?, user? }` lists the inactive objects belonging to given packages — ADT's `/sap/bc/adt/activation/inactiveobjects` resource is a worklist scoped to ONE USER with no package attribute at all, so abapsmith fetches that user's worklist (the caller by default, or `user`) and intersects it client-side with each package's contents (sub-packages too with `include_subpackages`); `query` is optional here where it's required everywhere else, `packages` is required here (1-5) where it's optional everywhere else. `abap_activate { package, recursive? }` activates every inactive object of the caller's worklist belonging to that package (and sub-packages with `recursive`) in one activation request so the server orders the dependencies; pending deletions are skipped and reported, DDIC types go first, and the existing DDIC fan-out chunking (`ABAP_MAX_DDIC_ACTIVATION_BATCH`) still splits large DDIC sets into consecutive requests; more than 50 inactive objects is `BAD_INPUT` (activate a subset with `objects` instead), and `package` cannot be combined with `object`/`objects`/`type`/`source`/`affects`/`corr_nr` (the transport comes from the objects' own locks, as in the `objects` batch form). And `abap_write`'s `CHECK_FAILED` now carries `inactive_dependencies: [{name, type, uri?}]` when a save and syntax check both succeeded but activation failed because OTHER objects were still inactive, naming them in the message and pointing at `abap_activate objects=[...]` or `abap_activate package=<package>` to fix it — derived from the activation reply, no extra request. Verified live on A4H 2026-09-23: with table `ZAS_T217` and class `ZCL_AS_T217` saved inactive in `$TMP`, `abap_search { inactive: true, packages: ["$TMP"] }` listed exactly those two (the class's inactive sub-includes folded into the one `CLAS/OC` row); in sub-package `ZAS_PKG213` under `ZTMA_COURSES` holding the same two objects, the listing showed both, `abap_activate { package: "ZAS_PKG213" }` activated both in one activation request (both clean, one transport), and the listing was empty afterwards; `{ package, object }` was refused `BAD_INPUT` with `details.stray`. `inactive_dependencies` was NOT reproduced live: three attempts to make an activation fail on another inactive object stopped earlier (syntax check or nametab errors) and no A4H reply carried `ioc:inactiveObjects`, so that path is covered by offline tests against the activation-reply shape the existing `CHECK_FAILED` parsing already reads.
