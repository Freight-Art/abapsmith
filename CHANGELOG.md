...
- `abap_debug` exception breakpoints (#152): `start` resolves the exception class first and refuses with `BAD_INPUT`, naming the class, before any breakpoint request when it does not exist — SAP accepts such a breakpoint and never fires it. An exception breakpoint the server accepted but did not echo back is reported in the `start` response as `NOT armed`. A run that ends without any armed exception breakpoint ever suspending it says so at death, with the line/statement-breakpoint alternative that is live-verified to stop at the raise. `test/integration-debug.test.ts` gains a live case for "exception breakpoint alone suspends before the dump" against a `$TMP` probe class (`ZCL_AS_DBGEXC`, source in the file); it skips when the class is absent and has not yet been run live.
